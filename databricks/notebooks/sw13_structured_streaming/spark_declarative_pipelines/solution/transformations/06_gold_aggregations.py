...
def gold_review_summary():
    ...
    return (
        ...
        .withColumn("positive_rate", F.round(F.col("positive_count") / F.col("total_reviews") * 100, 2))
        .withColumn("negative_rate", F.round(F.col("negative_count") / F.col("total_reviews") * 100, 2))
        .withColumn("rating_rounded", F.round(F.col("avg_rating"), 1))
